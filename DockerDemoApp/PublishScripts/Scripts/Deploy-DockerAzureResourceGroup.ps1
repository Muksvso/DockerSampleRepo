...
Param(
    ...
    [string] $TemplateFile = "..\Templates\DockerDemoAppDockerVM.json",
    [string] $TemplateParametersFile = "..\Templates\DockerDemoAppDockerVM.param.dev.json",
...
try {
    ...
    [Microsoft.Azure.Common.Authentication.AzureSession]::ClientFactory.AddUserAgent("VSAzureTools-$($UI)HostInCloudDocker$($host.name)".replace(" ","_"), "2.8.2")
} catch { }
